...
public class GoldoniumArmor extends DefaultArmor implements IHasModel {
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack armor) {
        ...
        if (player.getItemStackFromSlot(EntityEquipmentSlot.HEAD).getItem() instanceof GoldoniumArmor) {
            player.addPotionEffect(new PotionEffect(MobEffects.NIGHT_VISION, 10, 0, false, false));
        }

        if (player.getItemStackFromSlot(EntityEquipmentSlot.HEAD).getItem() instanceof GoldoniumArmor
                && player.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() instanceof GoldoniumArmor
                && player.getItemStackFromSlot(EntityEquipmentSlot.LEGS).getItem() instanceof GoldoniumArmor
                && player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem() instanceof GoldoniumArmor) {
