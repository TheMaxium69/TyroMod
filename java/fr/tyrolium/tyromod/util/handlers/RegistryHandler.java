...
public class RegistryHandler {
    ...
    public static void onPlayerTick(TickEvent.PlayerTickEvent e)
    {
        ...
        if(e.player.world.getBlockState(new BlockPos(player.posX, player.posY - 1, player.posZ)).getBlock() == BlocksMod.ELEVATOR_BLOCK)
        {
            if(player.isSneaking())
            {
                for(int i = (int)(player.posY - 2); i > 0; i--)
                {
                    if(e.player.world.getBlockState(new BlockPos(player.posX, i, player.posZ)).getBlock() == BlocksMod.ELEVATOR_BLOCK)
                    {
    ...
    public static double getElevatorJump(EntityPlayer player)
    {
        ...
        for(int i = (int)player.posY + 1; i < 256; i++)
        {
            ...
            if(player.world.getBlockState(pos).getBlock() == BlocksMod.ELEVATOR_BLOCK)
            {
                u = i;
                return i;
            }
    ...
    public static void playerJumpingEvent(LivingEvent.LivingJumpEvent e)
    {
        if(e.getEntityLiving() instanceof EntityPlayer)
        {
            ...
            if(player.world.getBlockState(new BlockPos(player.posX, player.posY - 1, player.posZ)).getBlock() == BlocksMod.ELEVATOR_BLOCK)
            {
