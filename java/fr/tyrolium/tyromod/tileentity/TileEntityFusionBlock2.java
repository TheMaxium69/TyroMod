...
import fr.tyrolium.tyromod.blocks.FusionBlock2;
...
public class TileEntityFusionBlock2 extends TileEntity implements IInventory, ITickable
{
    ...
    public static boolean isBurning(TileEntityFusionBlock2 te)
    {
    ...
    public void update() {
        if (!world.isRemote) {
            if (this.isBurning()) {
                --this.burnTime;
                FusionBlock2.setState(true, world, pos);
            }
            ...
            if (!fuel.isEmpty() && !this.handler.getStackInSlot(0).isEmpty() && !this.handler.getStackInSlot(1).isEmpty() && cookTime == 0) {
                ...
                if (FusionRegistry.verifFusion("2", inputs[0].getUnlocalizedName(), inputs[1].getUnlocalizedName())) {

                    /* REQUETE POUR LA FUSION */
                    ItemStack output = FusionBlock2Recipes.getInstance().getSinteringResult(inputs[0], inputs[1]);
            ...
            if (this.isBurning() && cookTime > 0) {
                if (cookTime == totalCookTime || cookTime > totalCookTime) {
                    TyroLogger.logServerFusion("2", "✅ Fusion effectuée avec succès : obtention de " + smelting.getUnlocalizedName()+ " dans le World : " + world.getWorldInfo().getWorldName() + " à x:" + pos.getX() + " y:" + pos.getY() + " z:" + pos.getZ());
                ...
                if (!this.handler.getStackInSlot(0).isEmpty() && !this.handler.getStackInSlot(1).isEmpty() && cookTime == 0) {
                    ...
                    if (FusionRegistry.verifFusion("2", inputs[0].getUnlocalizedName(), inputs[1].getUnlocalizedName()) && this.isBurning()) {

                        /* REQUETE POUR LA FUSION */
                        ItemStack output = FusionBlock2Recipes.getInstance().getSinteringResult(inputs[0], inputs[1]);
                        if (this.canSmelt(output)) {
                            if (!output.isEmpty()) {
                                TyroLogger.logServerFusion("2", "ℹ️ Fusion démarré (" + inputs[0].getUnlocalizedName() + " + " + inputs[1].getUnlocalizedName() + " = " + output.getUnlocalizedName() + ") dans le World : " + world.getWorldInfo().getWorldName() + " à x:" + pos.getX() + " y:" + pos.getY() + " z:" + pos.getZ());
...
//                if (block == Blocks.WOODEN_SLAB) return 150;
            ...
            if (item == ItemsMod.items[416]) return 201;
